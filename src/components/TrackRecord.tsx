...
import { CheckCircle, Calendar, Users, TrendingUp, Lightbulb } from "lucide-react";
            ...
            {timeline.map((item, index) => (
              <div key={index} className="relative mb-12 last:mb-0">
                <div className={`flex items-start ${index % 2 === 0 ? 'md:flex-row' : 'md:flex-row-reverse'}`}>
                  ...
                  {/* Content */}
                  <Card className={`ml-12 md:ml-0 md:w-5/12 ${index % 2 === 0 ? 'md:mr-auto md:pr-8' : 'md:ml-auto md:pl-8'}`}>
          ...
          <Card className="bg-gradient-to-r from-gradient-from/5 to-gradient-to/5 border-gradient-from/20">
            <CardContent className="p-8">
              <blockquote className="text-xl md:text-2xl font-semibold text-gray-800 mb-4">
                "We've been training people in AI thinking skills since before ChatGPT existed. Now we're ready to scale globally."
              </blockquote>
              <cite className="text-gray-600 font-medium">— TrainingX.Ai Team</cite>
            </CardContent>
          </Card>
        </div>
      </div>
