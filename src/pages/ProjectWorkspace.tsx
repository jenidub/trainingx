...
import { Step, MultipleChoiceOption, Project as ProjectType } from "@shared/schema";
...
export default function ProjectWorkspace() {
  ...
  const completeProject = async () => {
    ...
    const newSkills = { ...userState.skills };
    const skillsGained: string[] = [];
    project.buildsSkills.forEach(skill => {
      const oldValue = newSkills[skill as keyof typeof newSkills];
      const newValue = Math.min(100, oldValue + Math.round(ps * 0.2));
      if (newValue > oldValue) {
        skillsGained.push(skill);
      }
      newSkills[skill as keyof typeof newSkills] = newValue;
    });
    ...
    try {
      ...
      setLocation(`/practice/${project.slug}/result`);
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="flex flex-col lg:flex-row gap-6 mt-[20px] mb-[20px]">
          {/* Challenge Info - Left */}
            ...
            <Card className="h-full">
              <CardContent className="p-6">
                {currentStepData?.type === 'multiple-choice' ? (
                  <>
                    <h3 className="text-lg font-semibold mb-4">Select the Best Prompt</h3>
