...
import { insertCustomGPTSchema, type SelectCustomGPT } from "@shared/schema";
...
export default function CustomGPTs() {
  ...
  const [editingGPT, setEditingGPT] = useState<SelectCustomGPT | null>(null);
  const [chatGPT, setChatGPT] = useState<SelectCustomGPT | null>(null);
  ...
  const handleUpdate = async (gptId: string, data: any) => {
    try {
  ...
  const chatMutation = useMutation({
    mutationFn: async ({ id, messages }: { id: string; messages: ChatMessage[] }) => {
      const response = await apiRequest("POST", `/api/custom-gpts/${id}/chat`, { messages });
  ...
  }, [editingGPT, userId, form]);

  const handleSubmit = (data: any) => {
    if (editingGPT) {
      handleUpdate(editingGPT._id, data);
    } else {
      handleCreate(data);
    }
  };
  ...
  const handleSendMessage = () => {
    if (!chatInput.trim() || !chatGPT || chatMutation.isPending) return;

    const userMessage: ChatMessage = { role: "user", content: chatInput };
    const newMessages = [...chatMessages, userMessage];
    setChatMessages(newMessages);
    setChatInput("");
    chatMutation.mutate({ id: chatGPT.id, messages: newMessages });
  };
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              <h1 className="text-3xl font-bold tracking-tight" data-testid="text-custom-gpts-title">Custom Agent Builder</h1>
            ...
            <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
              <DialogTrigger asChild>
                <Button className="bg-gradient-to-r from-gradient-from to-gradient-to" data-testid="button-create-gpt">
      ...
      {isLoading ? (
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <Button onClick={() => setIsCreateOpen(true)} data-testid="button-create-first-gpt">
              <Plus className="mr-2 h-4 w-4" />
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {customGPTs.map((gpt) => (
            <Card key={gpt.id} data-testid={`card-gpt-${gpt.id}`}>
              <CardHeader>
              ...
              <CardFooter className="flex gap-2">
                <Button
                  ...
                  data-testid={`button-chat-${gpt.id}`}
                >
                ...
                <Button
                  ...
                  data-testid={`button-edit-${gpt.id}`}
                >
                ...
                <Button
                  ...
                  disabled={false}
                  data-testid={`button-delete-${gpt.id}`}
                >
                  ...
                  <FormItem>
                    ...
                    <FormControl>
                      <Input
                        placeholder="e.g., Marketing Assistant"
                        {...field}
                        data-testid="input-edit-gpt-name"
                      />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                  ...
                  <FormItem>
                    ...
                    <FormControl>
                      <Textarea
                        ...
                        rows={6}
                        {...field}
                        data-testid="input-edit-gpt-instructions"
                      />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
      ...
      <Dialog open={!!chatGPT} onOpenChange={(open) => !open && setChatGPT(null)}>
        <DialogContent className="max-w-2xl h-[600px] flex flex-col">
          <DialogHeader>
            ...
            <DialogDescription>{chatGPT?.instructions}</DialogDescription>
          </DialogHeader>
