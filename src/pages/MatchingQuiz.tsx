...
export default function MatchingQuiz() {
  ...
  const handleStartQuiz = () => {
    trackEvent('matching_quiz_start');
    setStep('questions');
  };
  ...
  const handleNext = () => {
    if (currentQuestion < matchingQuestions.length - 1) {
      ...
      trackEvent('matching_quiz_complete', { answers });
      saveResults();
      setStep('processing');
        ...
        <Card className="max-w-md w-full">
          <CardContent className="p-8 text-center">
            ...
            <p className="text-muted-foreground mb-6">
              Finding your perfect AI career matches
            </p>
            <Progress value={100} className="mb-4" />
            <p className="text-sm text-muted-foreground">
              Redirecting you to your personalized matches...
            </p>
          </CardContent>
        </Card>
      </div>
