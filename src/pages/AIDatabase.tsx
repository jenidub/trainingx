...
import { Briefcase, Building2, DollarSign, Wrench, Lock, CheckCircle, TrendingUp, MapPin, Home, ArrowRight, Database } from "lucide-react";
...
export default function AIDatabase() {
  ...
  const unlockedCount = allMatches.filter(m => 
    meetsRequirements(m, userState.promptScore, userState.skills, userState.completedProjects.length)
  ).length;

  const renderMatchCard = (match: typeof allMatches[0], isUnlocked: boolean) => {
    const Icon = categoryIcons[match.type as keyof typeof categoryIcons] || Briefcase;
    const colorClass = categoryColors[match.type as keyof typeof categoryColors];
    ...
    const missingSkills = match.skillThresholds
      ? Object.entries(match.skillThresholds)
          .filter(([skill, threshold]) => userState.skills[skill as keyof typeof userState.skills] < threshold)
          .map(([skill]) => skill)
      : match.requiredSkills.filter(
          skill => userState.skills[skill as keyof typeof userState.skills] < 60
        );

    return (
      <Card key={match.title} className={isUnlocked ? "border-l-4 border-l-green-500" : ""}>
        <CardHeader>
        ...
        <CardContent>
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-3 text-sm">
              {match.location && (
                <div className="flex items-center gap-1 text-gray-600" data-testid={`text-location-${match.careerId}`}>
                  <MapPin className="h-4 w-4" />
                  <span>{match.location}</span>
                </div>
              )}
              {match.remotePolicy && (
                <div className="flex items-center gap-1 text-gray-600" data-testid={`text-remote-${match.careerId}`}>
            ...
            {/* Salary */}
            {match.salaryRange && (
              <div className="p-3 bg-gradient-to-r from-gradient-from/10 to-gradient-to/10 rounded-lg" data-testid={`text-salary-${match.careerId}`}>
                <div className="font-semibold text-gray-800">{match.salaryRange}</div>
              </div>
            )}
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between text-sm">
                <span className="text-gray-600">Required PS:</span>
                <span className={userState.promptScore >= (match.requiredPS || 0) ? "text-green-600 font-medium" : "text-gray-800"}>
                  {match.requiredPS}+
                </span>
              </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1">
                    {match.requiredSkills.map(skill => {
                      const hasSkill = userState.skills[skill as keyof typeof userState.skills] >= 60;
                      return (
                        <Badge 
                          key={skill} 
                          variant={hasSkill ? "default" : "outline"}
                          className={`text-xs capitalize ${hasSkill ? 'bg-green-500' : ''}`}
                        >
                          {skill.replace(/_/g, ' ')}
                        </Badge>
                      );
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div className="text-sm text-blue-700">
                      ...
                      {missingSkills.length > 0 && (
                        <div>Build: {missingSkills.map(s => s.replace(/_/g, ' ')).join(', ')}</div>
                      )}
                    </div>
                  </div>
                </div>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            Browse our complete collection of {allMatches.length} AI career opportunities. {unlockedCount} unlocked based on your current skills.
          </p>
        </div>
                ...
                <CardContent className="p-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-sm text-gray-600 capitalize">{categoryLabels[category as keyof typeof categoryLabels]}</div>
                    </div>
                  </div>
                </CardContent>
              </Card>
          ...
          {Object.entries(matchesByCategory).map(([category, matches]) => (
            <div key={category}>
              <h2 className="text-xl font-bold mb-4 capitalize flex items-center gap-2">
                ...
                {categoryLabels[category as keyof typeof categoryLabels]} ({matches.length})
              </h2>
              <div className="grid md:grid-cols-2 gap-4">
                {matches.map(match => {
                  const isUnlocked = meetsRequirements(match, userState.promptScore, userState.skills, userState.completedProjects.length);
            ...
            <Card className="bg-blue-50 border-blue-200">
              <CardContent className="p-8 text-center">
                ...
                <p className="text-gray-600 mb-4">
                  Based on your quiz preferences, we couldn't find matching opportunities. Try adjusting your preferences.
                </p>
