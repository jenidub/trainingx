...
export default function Leaderboard() {
  ...
  const convexSortBy = sortBy === "practice" ? "promptScore" : sortBy === "community" ? "communityScore" : "totalScore";
  ...
  const userRank = useQuery(
    ...
    user?._id ? { userId: user._id as any, sortBy: convexSortBy as any } : "skip"
  );
  
  // Map Convex data to LeaderboardUser format
  const sortedUsers: LeaderboardUser[] = (leaderboardData || []).map((entry, index) => ({
    id: index + 1,
    name: entry.userName,
  ...
  const topUsers = sortedUsers.filter((u) => !u.isCurrentUser).slice(0, 10);
